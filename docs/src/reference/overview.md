...
`equate_image_sizes()` takes in an input directory path, and scans through the directory to find the maximum, and minimum image dimensions. Then, using the equalization method of choice (pad or crop), the function equalizes the image dimensions of all the images within the directory (in-place). This function can be used as a preprocessing step to `moviefy_images()`, or you can perform both steps in a single call by specifying `equate_sizes = True` in the `moviefy_images()` call. 
