...
def layer_colour(time_onset:float=None, time_offset:float=None, timing_function:Callable[...,float]=timing_linear, 
                 region_of_interest:list[list[tuple]] = [FACE_OVAL_PATH], fade_duration:int = 500, focus_color:str|int = "red", 
                 magnitude:float = 10.0, min_tracking_confidence:float = 0.5, min_detection_confidence:float = 0.5, **kwargs) -> LayerColour:
    
    return LayerColour(time_onset, time_offset, timing_function, region_of_interest, fade_duration, focus_color, magnitude, min_tracking_confidence, min_detection_confidence, **kwargs)