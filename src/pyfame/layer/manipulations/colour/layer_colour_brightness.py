...
def layer_colour_brightness(time_onset:float=None, time_offset:float=None, timing_function:Callable[...,float]=timing_linear, region_of_interest:list[list[tuple]] = [FACE_OVAL_PATH], 
                 fade_duration:int = 500, magnitude:float = 20.0, min_tracking_confidence:float = 0.5, min_detection_confidence:float = 0.5, **kwargs) -> LayerColourBrightness:
    
    return LayerColourBrightness(time_onset, time_offset, timing_function, region_of_interest, fade_duration, magnitude, min_tracking_confidence, min_detection_confidence, **kwargs)