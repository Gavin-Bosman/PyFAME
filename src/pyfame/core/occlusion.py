    ...
    for file in files_to_process:
        ...
        if not static_image_mode:
            ...
            if not capture.isOpened():
                logger.error("Function encountered a FileReadError when attempting to instantiate cv2.VideoCapture.")
                debug_logger.error(f"Function encountered an FileReadError when instantiating cv2.VideoCapture with file {file}.")
                raise FileReadError()
            ...
            if not result.isOpened():
                logger.error("Function encountered an FileWriteError when attempting to instantiate cv2.VideoWriter. ")
                debug_logger.error(f"Function encountered an FileWriteError when instantiating cv2.VideoWriter at file location {dir_file_path}.")
